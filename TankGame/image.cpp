...
void image::alphaRender(HDC hdc, int destX, int destY, BYTE alpha)
{
	...
	if (_isTrans)//배경색(마젠타) 없엔후 알파블렌드 할거냐?
	{
		//1.출력해야할 DC에 그려져 있는 내용을 블랜드 이미지에 그려준다.
		BitBlt(_blendImage->hMemDC,
			0, 0, _imageInfo->width, _imageInfo->height,
			hdc, destX, destY,
			SRCCOPY);
		//2.출력해야할 이미지를 블렌드이미지에 그려준다(마젠타값을 없엔다.)
...
void image::frameRender(HDC hdc, int destX, int destY)
{
	if (_isTrans) //배경색 없앨꺼냐?
	{
		...
		GdiTransparentBlt(
			...
			_imageInfo->hMemDC,			//복사될 대상 DC
			_imageInfo->currentFrameX * _imageInfo->frameWidth,	//렌더를 호출하면서 커런트프레임이 계속 바껴야 한다.
...
void image::frameRender(HDC hdc, int destX, int destY, float scale)
{
	if (_isTrans) //배경색 없앨꺼냐?
	{
		...
		GdiTransparentBlt(
			...
			destX,						//복사될 좌표 시작점 x
			destY,						//복사될 좌표 시작점 y
			_imageInfo->frameWidth * scale,			//복사될 이미지 가로크기
			_imageInfo->frameHeight * scale,			//복사될 이미지 세로크기
			_imageInfo->hMemDC,			//복사될 대상 DC
			_imageInfo->currentFrameX * _imageInfo->frameWidth,	//렌더를 호출하면서 커런트프레임이 계속 바껴야 한다.
			_imageInfo->currentFrameY * _imageInfo->frameHeight,						//복사 시작지점
			_imageInfo->frameWidth,			//복사 영역 가로크기
...
{//LPRECT == Long Pointer RECT
	...
	for (int y = 0; y < drawAreaH; y += sourHeight)
	{
		...
		//소스영역이 그리는 화면을 넘어갔다면(화면밖으로 나간경우)
		if (y + sourHeight > drawAreaH)
		{
			//넘어간 그림의 값만큼 바텀값을 빼준다.
